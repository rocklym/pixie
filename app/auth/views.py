...
def logout():
    logout_user()
    flash('You are gone.I will miss you~')
    return redirect(url_for('main.index'))
...
def confirm(token):
    ...
    if current_user.confirm(token):
        ...
        flash('The confirmation link is valid or has expired.')
    return redirect(url_for('main.index'))
...
def change_email_request():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.password.data):
            ...
            send_email(new_email, 'Change Your Email', 'auth/email/change_email', user=current_user, token=token)
            flash('A email with instructions to change your email address has been sent to you')
            return redirect(url_for('main.index'))
        else:
            flash('Invalid password')
