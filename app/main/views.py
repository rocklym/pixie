...
def index():
    ...
    pagination = Post.query.order_by(Post.timestamp.desc()).paginate(page, per_page=25, error_out=False)
...
def follow(username):
    ...
    if current_user.is_following(user):
        flash('You are already following this gay.')
...
def unfollow(username):
    ...
    if not current_user.is_following(user):
        flash('You are not following this gay.')
        return redirect(url_for('.user', username=username))
